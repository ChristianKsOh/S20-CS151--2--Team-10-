...
public class FormView extends UIPanel{
	...
	String[] daisies = new String[25];
	String[] flowers = new String[3];
	String[] daisies2 = new String[25];
	String[] flowers2 = new String[11];
	int THIscore;
	int TFIscore;
	Patient p;
	...
	public FormView() {
		...
		thi.makeForm();		//make questions
		thi.makeAnswers();	//make answers
		tfi.makeForm();
		tfi.makeAnswers();
		...
		for(int i=0;i<thi.getQuestionSize();i++) {
			THIquestions[i] = new JLabel((i+1) + ". " + thi.getQuestion(i));
			TFIquestions[i] = new JLabel((i+1) + ". " + tfi.getQuestion(i));
			//grid coordinates
			gbc.gridx = 0;
			gbc.gridy = i+1;
			...
			//THI - 3 answers
			for(int j=0;j<thi.getAnswerSize();j++) {
				//grid coordinates
				gbc.gridx = j+1;
				gbc.gridy = i+1;
				THIanswers[j] = new JRadioButton(thi.getAnswer(j));		//create new radiobutton
			...
			//TFI - 10 answers
			for(int k=0;k<tfi.getAnswerSize();k++) {
				gbc.gridx = k+1;
				gbc.gridy = i+1;
				if(i == 0 || i == 2)		//percentages for q1 and q3
					TFIanswers[k] = new JRadioButton("" + (k*10) + "%");
				else
					TFIanswers[k] = new JRadioButton(tfi.getAnswer(k));
